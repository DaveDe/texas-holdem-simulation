 ...
 */
    public static void main(String[] args) {
        ...
        for(int i = 0; i < 1000000; i++){
            ...
            Display dis = new Display(winnerHand);
            String freqHand = dis.findFrequency();
                for(int j = 0; j < frequency.length; j++){
                    if(freqHand.equals(Rank.possibleHands[j])){
                        frequency[j]++;
                        break;
                    }
                }
        ...
        for(int i = 0; i < frequency.length; i++){
            System.out.println(Rank.possibleHands[i] + ": " + frequency[i]);
        }
    }
}
